/* Name: Steven Song  
   UVA ID: sqs5hrn 
   Date: March 24, 2021  
   CS2150 Lab 5 Post Lab AVLTree.h  
*/

#ifndef AVL_H
#define AVL_H

#include <string> // Necessary for string operations/variables 
#include "AVLNode.h"
...
private:
    ...
    // AVLNode* insert2(AVLNode*& n, const string& x); 
    void insert2(AVLNode*& n, const string& x); // helper method for insert() 
    int numNodes2(AVLNode* n) const; // helper method for numNodes() 
    string pathTo2(AVLNode* n, const string& x) const; // helper method for pathTo() 
    bool find2(AVLNode* n, const string& x) const; // helper method for find() 
};
