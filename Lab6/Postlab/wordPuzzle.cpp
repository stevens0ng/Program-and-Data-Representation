/* Name: Steven Song 
   UVA ID: sqs5hrn 
   Date: March 30, 2021 
   CS2150 Lab 6 Post Lab wordPuzzle.cpp 
*/
...
int main(int argc, char* argv[]) {
  ...
  hashTable* hashtabledictionary = new hashTable(dictionaryfilelength);
  ...
  for(int w = 0; w < rows; w++) {
	...
	for (int z = 3; z < lengthmax; z++) {
	  ...
	  if (hashtabledictionary -> Findhashword(wordchecker) == wordchecker) { // Only having to use the find method instead of having to check for both the find and check methods already reduces the run time in half 
