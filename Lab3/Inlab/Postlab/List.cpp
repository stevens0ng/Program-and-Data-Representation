/* Name: Steven Song
   ...
   CS2150 Lab 3 Post Lab List.cpp
*/
...
void List::makeEmpty() {
  ...
  while (!firstelement.isPastEnd()) { // If the first element is not past the end of the linked list, then you have to loop through it and delete nodes one by one starting from the first element
    ...
    n = firstelement.current;
    // n = firstelement.current;
    // iterate through the list  // currrent is a pointer to a list node 
    // remove(firstelement.current -> value); // it should adjust the pionter
    // Since n is a pointer, you have to do n -> remove instead of n.remove()
    // we don't have the remove method in ListNode
  ...
  } // You're creating ListNode n (Inside the while loop you're already deleting, and not instantiating it)  
...
void List::insertAfter(int x, ListItr position) { // insert after is almost the same as insert before except you have to switch the order of next and previous
  ...
  count++; // count is the number of nodes you have so you have to increment count at the e
} 
...
void List::insertAtTail(int x) { // x is a value 
  ListNode *n = new ListNode(); // Creating a new node (DYNAMICALLY allocating it) 
...
// Returns an iterator that points to the first occurrence of x (MUST MODIFY FOR THE LAB 3 POST LAB)
// This find method is different than what it was in Lab 2 (you start from the tail instead of the head since we only have an insertAtTail method in the List.cpp file and not an insertAtHead method)  
ListItr List::find(int x) {
  // Uses the data structure that you make in another file to make something
  
  // ListItr iter (head -> next); // top of your stack
  ListItr iter (tail -> previous); // Since we're calling the insertAtTail method for the push method, we have to start backwards for the iter at tail instead of head before  
  while (iter.retrieve() != x && !(iter.isPastEnd())) {  
    // iter.moveForward(); // If you're not able to retrieve x, then keep searching for iter
    iter.moveBackward(); 
  }
  if (iter.current == NULL) {
    // iter.current = tail;
    iter.current = tail -> previous; // You've iterated through the whole linked list and don't find it. IF it's not there, then it will keep iterating to the tail.
    // The tail is the second dummy node and it points to NULL
  }
  return iter; // If you're finding something you don't need the count.
}   

// Get a pointer to the value that's before tail: pass that value into the remove method. THEn call in the moreve method to that method

// If the head is at the top of your stack: make sure that you're starting your find at head (if you start at tail, and the value you're duplicated in your stack, you'll delete the wrong one)
...
void printList(List& source, bool forward) { // NOT A MEMBER/METHOD OF THE LIST CLASS
  if (forward == true) { // Printing the list fowards 
